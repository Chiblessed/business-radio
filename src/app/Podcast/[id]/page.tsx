  ...
  const cleanDes = shortenedPodcastDesc.replace(/<br\s*\/?>/gi, '');
    ...
    <>
      <main className='h-full'>
        <div 
        className='bg-opacity-50 bg-gradient-to-br from-[#2B3221] via-[#2B3221] to-[#F2F2F200] px-10 sm:px-3 max-sm:px-3 max-sm:flex-col sm:flex flex items-center justify-center gap-5 relative py-6'>
            <Image src={podcast.picture_url} width={400} height={400} alt='' />
            <div className='flex flex-col max-sm:items-center sm:items-start'>
                ...
                <p className='text-base font-medium mb-14 text-white w-[60%] leading-[26px] max-sm:w-full sm:w-full'>
                {cleanDes}
                    </p>
...
<rect width="24" height="24" fill="white" transform="translate(7 8)"/>
        ...
        {/*more podcast **/}
        <div className='flex items-start justify-between gap-44 px-10 mt-20 max-sm:flex-col sm:flex'>
            ...
            return (
              <div key={episode.id} onClick={() => handleNavigate(episode.id)}
                className='cursor-pointer max-sm:flex-col sm:flex-col flex items-start justify-start gap-3 py-3 border-t border-t-[#DCDCDC]'>
                <Image src={podcast.picture_url} width={100} height={100} alt='' className='sm:w-[20rem] max-sm:w-[20rem]'/>
                <div className='flex flex-col items-start'>
                  ...
                  <p className='text-[15px] font-medium text-[#282828] w-[75%] pb-3 max-sm:w-full sm:w-full'>
                    {cleanTitle}
                  </p>
...
<rect width="18" height="18" fill="white"/>
  ...
  {/* Pagination **/}
                  <div className='flex items-start justify-start gap-2 text-[15px]  text-white font-medium ml-[10%] my-20 sm:ml-0 max-sm:ml-0'>
                    <button className='px-3 py-2 bg-[#AEAEAE] rounded-lg'
                        ...
                        {[...Array(totalPages)].map((_, index) => {
                            ...
                            return (
                                <button
                                    ...
                                    className={`px-3 py-1 bg-[#AEAEAE] rounded-lg ${currentPage === pageNumber ? 'bg-[#2C2C2C] text-white ' : 'bg-[#AEAEAE]' }`}
                                >
                                    {pageNumber}
                                </button>
                            );
