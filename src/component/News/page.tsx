...
function News() {
    const [newsVisible] = useState<number>(5); 
    const router = useRouter();

    const fetchNews = async (): Promise<PodcastItem[]> => {
      try {
    ...
    for (let i = 0; i < getData.length; i++) {
      ...
      if (result.length === newsVisible) break;
    }
  ...
  return (
    <>
     <main className='mx-14 pb-20  max-sm:mx-4 '>
...
<path d="M1 9L6 5L1 1" stroke="#9747FF" stroke-linecap="round" stroke-linejoin="round"/>
     ...
     {isLoading ?  <div className="flex justify-center items-center h-1/2">
      <div className="animate-spin rounded-full h-12 w-12 border-t-4 border-b-4 border-[#5A5A5A]"></div>
        ...
        <div className='flex items-start justify-start max-sm:flex-col gap-8 mt-10  max-md:grid max-md:grid-cols-1 overflow-hidden'>

        {news?.map((item, index) => (
            <div key={index} 
            onClick={() => handleNavigate(item.id)}
            className="bg-[#F4F4F4] flex flex-col max-sm:w-full max-sm:items-center items-start justify-start shadow-lg rounded-xl py-5 px-4">
