...
function Lifestyle() {
  ...
  const fetchData = async (): Promise<PodcastItem[]> => {
    try {
      ...
      for (let i = 0; i < getData.length; i++) {
        ...
        if (item.category_type === "SOCIETY & CULTURE") {
          result.push(item);
        }
  ...
  const { data: lifestyles = [], isLoading } = useQuery<PodcastItem[]>({
    queryFn: fetchData,
    queryKey: ['lifestyles'],
  });
...
<path d="M1 9L6 5L1 1" stroke="#9747FF" stroke-linecap="round" stroke-linejoin="round"/>
     ...
     {isLoading ?  <div className="flex justify-center items-center h-1/2">
      <div className="animate-spin rounded-full h-12 w-12 border-t-4 border-b-4 border-[#5A5A5A]"></div>
        ...
        <div className='flex items-start justify-start gap-7 mt-10 max-sm:flex-col  max-md:grid max-md:grid-cols-1'>
            ...
            onClick={() => handleNavigate(item.id)}
             className='bg-[#F4F4F4] max-sm:items-center max-sm:w-full flex flex-col items-start justify-start shadow-lg rounded-xl py-5 px-4'>
