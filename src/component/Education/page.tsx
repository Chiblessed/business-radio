...
function Education() {
  const [educationVisible] = useState<number>(5); 
  const router = useRouter();

  const fetchEducation = async (): Promise<PodcastItem[]> => {
    try {
      ...
      for (let i = 0; i < getData.length; i++) {
        ...
        if (item.category_type === "EDUCATION") {
          result.push(item);
        }
        if (result.length === educationVisible) break;
      }
  ...
  const { data: educations = [], isLoading } = useQuery<PodcastItem[]>({
  ...
  return (
    <>
     <main className='mx-14 pb-14 max-sm:mx-4'>
...
=======
        <div className='flex items-start justify-start gap-7 mt-5 max-sm:flex-col  sm:grid sm:grid-cols-2'>
...
className='bg-[#F4F4F4] flex flex-col items-start justify-start max-sm:items-center max-sm:w-full  shadow-lg rounded-xl py-5 px-4'>
